...
import dev.magnetite.dodgeball.assemble.events.AssembleBoardCreatedEvent;
...
public class AssembleBoard {
	...
	/**
	 * Gets a player's bukkit scoreboard.
	 *
	...
	/**
	 * Gets the player's scoreboard objective.
	 *
	...
	public Objective getObjective() {
		...
		if (scoreboard.getObjective("Assemble") == null) {
			...
			objective.displayName(assemble.getAdapter().getTitle(Bukkit.getPlayer(uuid)));
			return objective;
		} else {
	 ...
	 * Set up the board.
	 *
	 * @param player who's board to set up.
	 */
	...
	public String getUniqueIdentifier(int position) {
		StringBuilder identifier = new StringBuilder(getRandomChatColor(position) + ChatColor.WHITE);

		while (this.identifiers.contains(identifier.toString())) {
			identifier.append(getRandomChatColor(position)).append(ChatColor.WHITE);
		}
